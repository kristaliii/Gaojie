...
public class CToast {  
    ...
    public static CToast makeText(Context context, CharSequence text,   int duration) {  
        ...
        LinearLayout mLayout = new LinearLayout(context);  // 吐司的布局
        TextView tv = new TextView(context);  
        tv.setText(text);  
        tv.setTextColor(Color.WHITE);  
        tv.setGravity(Gravity.CENTER);  
        tv.setTextSize(18);
        mLayout.setBackgroundResource(R.color.confirm);  
  
        int w = context.getResources().getDisplayMetrics().widthPixels / 2;  // 吐司的宽度
        int h = context.getResources().getDisplayMetrics().widthPixels / 5;  // 吐司的显示长度
