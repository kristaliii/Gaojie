 ...
 * 批发退货扫描
 */
public class pifatuihuoScanFragment extends BaseFragment {

    Button btnQuery; // 查询

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View view = View.inflate(getActivity(), R.layout.fragment_pifatuihuo_scan, null);
        initView(view);
        return view;
    }
...
//        getActivity().setTitle("采购扫描");
        ((MainActivity)getActivity()).setTitle("批发退货扫描");
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btnQuery: // 查询
                mFHelper.transcateFoward(FragmentFactory.pifatuihuoQuery_Fragment);
                break;
        }
    }
}
